...
class DockerContainerClient {
    ...
    async ping() {
        try {
            common_1.log.debug(`Pinging container runtime...`);
            const response = await (0, common_1.streamToString)(stream_1.Readable.from(await this.dockerode.ping()));
            common_1.log.debug(`Pinged container runtime`);
            return response;
        }
        catch (err) {
            common_1.log.warn(`Failed to ping container runtime: "${err}"`);
            throw err;
        }
    }
    getById(id) {
